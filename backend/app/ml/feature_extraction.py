...
def extract_features(cv_text: str, job_text: str) -> dict[str, float]:
    ...
    # --- Semantic features --------------------------------------------------
    semantic = compute_semantic_match(
        cv_text,
        job_text,
    ...
    education = match_education(candidate.education, job.education)
    certs = match_certifications(
    ...
    features: dict[str, float] = {
        ...
        "semantic_similarity": round(semantic.raw_score, 6),
        "experience_gap_years": round(experience_gap, 4),
        "experience_score": round(experience.overall_score, 6),
        "experience_data_available": 1.0 if experience_known else 0.0,
        "seniority_match": seniority_match,
        "education_level_score": round(education.level_score, 6),
        "education_field_score": round(education.field_score, 6),
        "certification_match_ratio": round(certs.score, 6),
